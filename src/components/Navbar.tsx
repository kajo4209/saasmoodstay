...
export function Navbar() {
  ...
  return (
    <nav id="main-navbar" className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${navBg}`}>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex flex-col justify-center items-center gap-[5px] w-9 h-9 rounded-xl transition-colors flex-shrink-0
              ${scrolled ? isDark ? "hover:bg-gray-800" : "hover:bg-gray-100" : "hover:bg-white/15"}`}
          >
          ...
          <Link href="/" className={textColor}><Logo /></Link>
        </div>
      ...
      <div className={`transition-all duration-300 overflow-hidden ${menuOpen ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className={`mx-3 mb-3 rounded-2xl shadow-2xl border ${isDark ? "bg-gray-900 border-gray-700/60" : "bg-white border-gray-100"}`}>
          ...
          {/* Hidden dashboard — مستطيل أزرق، ضغطة واحدة */}
          <div className={`flex justify-center py-3 ${isDark ? "border-t border-gray-800" : "border-t border-gray-50"}`}>
            <button
              onClick={() => { window.location.href = "/login"; }}
              aria-hidden="true"
              tabIndex={-1}
              className="w-16 h-1.5 rounded-sm bg-sky-500 opacity-30 hover:opacity-60 transition-opacity cursor-default select-none"
            />
          </div>
